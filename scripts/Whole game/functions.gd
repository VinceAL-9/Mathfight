...
func load_screen_to_scene(target: String) -> void:
	var loading_screen0 = preload("res://scenes/Whole game/loading_screen.tscn").instantiate()
	loading_screen0.next_scene_path = target
	get_tree().current_scene.add_child(loading_screen0)
